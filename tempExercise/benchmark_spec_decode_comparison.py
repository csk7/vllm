...
Requirements:
    ...
    - Model and draft model accessible (default: Qwen/Qwen3-8B and RedHatAI/Qwen3-8B-speculator.eagle3)
...
# Note: Qwen3-30B-A3B is a MoE model. If no 30B speculator is available, you can:
# 1. Use a smaller Qwen3 model's speculator (may have lower acceptance rate)
# 2. Train a custom speculator for 30B
# 3. Use a different 30B model that has a speculator (e.g., Qwen3-32B if available)
# For now, using 8B speculator as placeholder - update if you have a 30B speculator.
DRAFT_MODEL = "RedHatAI/Qwen3-30B-A3B-Instruct-2507-speculator.eagle3"  # TODO: Update to 30B speculator if available
PORT = 8000
BASE_URL = f"http://localhost:{PORT}"
LOG_DIR = Path("/workspace/siva_home/projects/vllm/log/spec_script/")
...
SCENARIOS = [
    ...
    {
        "name": "Scenario2_Server4_ClientUnlimited",
        "server_max_num_seqs": 4,
        "client_max_concurrency": 4,  # Unlimited client concurrency
    },
]
...
def wait_for_server(timeout: int = 120) -> tuple[bool, str | None]:
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # Extract the actual model name from the server
                if "data" in data and len(data["data"]) > 0:
                    actual_model_name = data["data"][0].get("id", None)
                    return True, actual_model_name
                return True, None
        except Exception:
            pass
        time.sleep(1)
...
def start_server(
    ...
    Returns:
        tuple: (process, actual_model_name) - The server process and the actual model name from server
    """
    cmd = [
        "vllm",
        "serve",
    ...
    is_ready, actual_model_name = wait_for_server()
    if is_ready:
        print("✓ Server is ready!")
        if actual_model_name:
            print(f"✓ Server model name: {actual_model_name}")
        return process, actual_model_name or MODEL
    else:
...
def run_benchmark(
    scenario_name: str,
    use_spec_decode: bool,
    client_max_concurrency: Optional[int],
    result_file: Path,
    ...
    Args:
        actual_model_name: The actual model name from the server (if None, uses MODEL)
    """
    ...
    cmd = [
        ...
        "--model", model_name,
        "--backend", "openai",
        "--endpoint", "/v1/completions",  # Explicitly set endpoint
        "--dataset-name", "random",
        "--random-input-len", str(RANDOM_INPUT_LEN),
        "--random-output-len", str(RANDOM_OUTPUT_LEN),
        "--num-prompts", str(NUM_PROMPTS),
        "--request-rate", str(REQUEST_RATE),
        "--num-warmups", str(WARMUP_REQUESTS),
        "--save-result",
        "--result-dir", str(LOG_DIR),
...
def create_comparison_plot(results: Dict, output_path: Path):
    ...
    fig.suptitle('Vanilla vs Speculative Decoding TPOT Comparison', fontsize=16, fontweight='bold')
    ...
    for metric_key, metric_title, ax in metrics_to_plot:
        ...
        for scenario_name in scenarios:
            if scenario_name in results:
                ...
                vanilla_data = results[scenario_name].get("vanilla", {}).get(metric_key, 0)
                spec_data = results[scenario_name].get("spec_decode", {}).get(metric_key, 0)
        ...
        bars1 = ax.bar(x - width/2, vanilla_values, width, label='Vanilla', color='#2E86AB', alpha=0.8)
        bars2 = ax.bar(x + width/2, spec_decode_values, width, label='Spec Decode', color='#A23B72', alpha=0.8)
...
def main():
    ...
    try:
        for scenario in SCENARIOS:
            ...
            server_process, actual_model_name = start_server(
            ...
            print(f"{'='*80}")
            print(f"Vanilla Mean TPOT: {results[scenario_name]['vanilla'].get('mean_tpot_ms', 'N/A')} ms")
            print(f"Spec Decode Mean TPOT: {results[scenario_name]['spec_decode'].get('mean_tpot_ms', 'N/A')} ms")
            if 'mean_tpot_ms' in results[scenario_name]['vanilla'] and 'mean_tpot_ms' in results[scenario_name]['spec_decode']:
                speedup = results[scenario_name]['vanilla']['mean_tpot_ms'] / results[scenario_name]['spec_decode']['mean_tpot_ms']
